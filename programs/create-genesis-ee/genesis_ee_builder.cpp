...
#define MAP_FILE_SIZE uint64_t(8*1024)*MEGABYTE
...
uint64_t genesis_ee_builder::read_hash(bfs::fstream& in) {
    uint64_t hash = 0;
...
void genesis_ee_builder::process_votes() {
    ...
    while (in) {
        ...
        auto vote_itr = vote_index.find(std::make_tuple(hash, vop.voter));
