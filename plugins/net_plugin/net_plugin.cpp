...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   {
      auto itr = my_impl->connections.find(connection_state::connected);
      for (; itr != my_impl->connections.upper_bound(connection_state::connected); ++itr) {
         if ((*itr)->connected()) {
            (*itr)->send_handshake();
         }
      }
   ...
   void dispatch_manager::bcast_block(const block_state_ptr& bs) {
      ...
      for (; itr != my_impl->connections.upper_bound(connection_state::connected); ++itr) {
         if (skips.find(*itr) != skips.end() || !(*itr)->current() ) {
            continue;
         }
         bool has_block = (*itr)->last_handshake_recv.last_irreversible_block_num >= bnum;
         if( !has_block ) {
            if (!(*itr)->add_peer_block(pbstate)) {
               continue;
            }
            if( !send_buffer ) {
               send_buffer = create_send_buffer( bs->block );
            }
            fc_dlog(logger, "bcast block ${b} to ${p}", ("b", bnum)("p", (*itr)->peer_name()));
            (*itr)->enqueue_buffer( send_buffer, true, priority::high, no_reason );
         }
      }
      ...
      if (msg.generation == 1) {
         ...
         if( c->peer_addr.empty() || c->last_handshake_recv.node_id == fc::sha256()) {
            ...
            for (auto itr = connections.lower_bound(connection_state::connected); itr != connections.end(); ++itr) {
               if (*itr == c)
                  continue;
               if ((*itr)->connected() && (*itr)->peer_name() == msg.p2p_address) {
                  // It's possible that both peers could arrive here at relatively the same time, so
                  // we need to avoid the case where they would both tell a different connection to go away.
                  // Using the sum of the initial handshake times of the two connections, we will
                  // arbitrarily (but consistently between the two peers) keep one of them.
                  if (msg.time + c->last_handshake_sent.time <= (*itr)->last_handshake_sent.time + (*itr)->last_handshake_recv.time)
                     continue;
   ...
   void net_plugin_impl::handle_message(const connection_ptr& c, const address_request_message& msg) {
      ...
      for (auto itr = connections.lower_bound(connection_state::connected); itr != connections.end(); ++itr) {
         if ((*itr)->connected() && private_peers.find((*itr)->peer_addr) == private_peers.end())
            amsg.addresses.push_back((*itr)->peer_addr);
      }
      c->enqueue(amsg);
   }

   void net_plugin_impl::handle_message(const connection_ptr& c, const address_message& msg) {
      const auto& idx = connections.get<by_addr>();
      for (const auto& host : msg.addresses) {
   ...
   void net_plugin_impl::fetch_addresses() {
      ...
      fetch_addresses_timer->async_wait( [this]( boost::system::error_code ec ) {
         app().post( priority::low, [this, ec]() {
            ...
            for (auto itr = connections.lower_bound(connection_state::connected); itr != connections.end(); ++itr) {
               if ((*itr)->connected() && (*itr)->protocol_version >= proto_address_advertising) {
                  (*itr)->enqueue(address_request_message());
               }
            }
         } );
      } );
   ...
   string net_plugin::connect( const string& host ) {
      if (my->connections.get<by_addr>().find(host) != my->connections.get<by_addr>().end())
